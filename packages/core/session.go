...
// Session is a persisted conversation transcript tied to a cwd.
type Session struct {
	ID     string
	Path   string
	Meta   SessionMeta
...
// NewSession creates and opens a session in root's configured backend.
func NewSession(root, cwd, providerName, model, version string) (*Session, error) {
	id := uuid.NewString()
	path, writer, err := createStoredSession(root, cwd, id)
	if err != nil {
		return nil, err
	}
	return newSession(path, writer, id, cwd, providerName, model, version)
}
...
func newSessionAt(p, cwd, providerName, model, version string) (*Session, error) {
	...
	return newSession(p, f, id, cwd, providerName, model, version)
}

func newSession(path string, writer io.WriteCloser, id, cwd, providerName, model, version string) (*Session, error) {
	s := &Session{
		ID:        id,
		Path:      path,
...
func OpenSession(path string) (*Session, []provider.Message, error) {
	...
	out, err := openSessionAppender(path)
	if err != nil {
		return nil, nil, err
	}
...
// SessionSummary describes one persisted session at a glance for UI pickers.
...
func RenameSession(path, title string) error {
	f, err := openSessionAppender(path)
	if err != nil {
		return err
	}
	defer f.Close()
...
func DeleteSession(path string) error {
	...
	return removeStoredSession(path)
}
...
func PruneEmptySessions(root, cwd string) {
	if paths, ok := listSQLSessions(root, cwd); ok {
		for _, path := range paths {
			if sessionHasNoMessages(path) {
				_ = removeStoredSession(path)
			}
		}
		return
...
// Close flushes and closes the session. If the session was freshly
// created in this process and never had any messages
